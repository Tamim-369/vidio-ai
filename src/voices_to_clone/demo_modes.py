...
def run(voice_id: str, text: str):
    print(f"\n{'='*60}\n{voice_id}\n{'='*60}", flush=True)
    subprocess.run(
        [sys.executable, "src/voices_to_clone/test_voice.py", "--voice", voice_id, "--out", f"mode_{voice_id}", text],
        check=True,
    )
...
if __name__ == "__main__":
    for vid, script in VOICE_SCRIPTS.items():
        run(vid, script)
    print("\nDone: voice_tests/mode_donald-trump.wav, mode_arnold-schwarzenegger.wav")