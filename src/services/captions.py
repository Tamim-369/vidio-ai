"""Word-level timing derived from a narration waveform.

The quote card shows a whole line at once for the whole line, so there is
nothing to burn in as subtitles any more. What still needs word timings is
tts_dsp._enforce_pauses(), which uses them to place a pause at a natural
sentence boundary instead of at an arbitrary offset — so speech keeps the
cadence the voice gave it after the audio is trimmed and padded.

The model is deliberately audio-forced rather than proportional: the waveform
is split into bursts at real silences, and each word's start is placed by
cumulative speech energy inside its burst. Long or loud words get more time, and
during a pause the current word holds rather than ticking along.
"""
