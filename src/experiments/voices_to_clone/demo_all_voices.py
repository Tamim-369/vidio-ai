...
def run(voice_id: str):
    keep = str(Path("src/experiments/voice_tests") / f"demo_{voice_id}.wav")
    print(f"\n{'='*60}\n{voice_id}\n{'='*60}", flush=True)
    subprocess.run(
        [sys.executable, "src/experiments/voices_to_clone/test_voice.py", "--voice", voice_id, "--out", f"demo_{voice_id}", SCRIPT],
        check=True,
    )
    print(f"-> {keep}")
