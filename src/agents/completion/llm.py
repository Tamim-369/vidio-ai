"""Every LLM call in the project: Groq (primary), Gemini, Cloudflare (last resort).

    call_text()  -> Groq (3 rotating keys) -> Gemini -> Cloudflare
    call_groq()  -> Groq (3 rotating keys) -> Gemini

Keys rotate on failure and pick up where the last call succeeded, so a dead key
is never re-hit first. Cloudflare is last because it rate-limits constantly.
Nothing else in the codebase builds a client or a retry loop.
"""
import os
import time
...
# Free and fast; gemma-4-26b blanked out often enough to be unusable here.
...
_primary = os.getenv("GEMINI_API_KEY", "")
if _primary and _primary not in GEMINI_KEYS:
    GEMINI_KEYS.append(_primary)
...
Retries only on rate-limit/413/token errors (backoff 1s, 2s, 4s): a retriable
failure rotates to the next Groq key, and once all keys are tried it falls back
to Gemini. Non-retriable failures advance keys immediately.

allow_fallback=False makes it Groq-only, raising instead of calling Gemini once
the keys are exhausted. The quote agent passes False, because Gemini output
would break its "quotes come from Groq" guarantee."""
