"""Local completion seam + output parsing for the asset agent.

Everything funnels through common.llm `_local` (Ollama) so the rest of the
pipeline's tuning (num_predict budget, wall-clock timeout, repeat_penalty)
applies unchanged. Strips thinking tags and digs a JSON block out of raw
output the same way the old Groq path did.
"""
