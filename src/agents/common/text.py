...
def _scalar(v) -> str:
    """Return a plain scalar string for grading (handles lists/dicts)."""
    if v is None:
        return ""
    if isinstance(v, (str, int, float)):
        return str(v).strip()
    if isinstance(v, list):
        return " ".join(_scalar(x) for x in v).strip()
    if isinstance(v, dict):
        return _scalar(next((v[k] for k in ("text", "value") if k in v), ""))
    return str(v).strip()