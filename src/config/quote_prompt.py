"""Prompt for the anti-wisdom quote agent.

The prompt text is the user's specification, reproduced verbatim. Do not edit
QUOTE_PROMPT casually: the generation rules, the format-rotation requirement and
the self-check list are the contract, and the agent re-sends rejected quotes
with feedback, so wording changes alter output quality.

Two additions live OUTSIDE the prompt text and are applied by the agent:

  * OUTPUT_FORMAT asks for one JSON object per line so a batch can be parsed
    unambiguously. The prompt's own "output only the final quotes" rule is kept
    in the text; the agent strips labels/meta-commentary itself rather than
    trusting the model to comply.
  * The agent supplies the already-used quotes and figures each round so
    rotation is enforced across videos, not just within one reply.
"""
